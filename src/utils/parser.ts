import { PlaceDetails, PlaceFormValues, PlaceType, ImageCredit } from "@/types/place";
...
export function normalizePlaceDetails(formValues: PlaceFormValues): PlaceDetails {
  ...
  const creditsStr = Array.isArray(formValues?.credits)
    ? JSON.stringify(formValues.credits)
    : typeof formValues?.credits === "string" && formValues.credits
      ? formValues.credits
      : JSON.stringify([]);
...
export function mapPlaceDetailsToFormValues(place: PlaceDetails): PlaceFormValues {
  ...
  if (place.credits) {
    try {
      const parsed = JSON.parse(place.credits);
      if (Array.isArray(parsed)) {
        credits = parsed.map((item, idx) => ({
          imageIndex: typeof item.imageIndex === "number" ? item.imageIndex : idx,
          imageUrl: item.imageUrl || (place.images && place.images[idx]) || "",
          author: item.author || "Unknown Author",
          authorUrl: item.authorUrl || "",
          source: item.source || "Direct Upload / Contributor",
      ...
      if (typeof place.credits === "string" && place.credits.trim()) {
        credits = (place.images || []).map((img, idx) => ({
          imageIndex: idx,
          imageUrl: img,
          author: place.credits,
          authorUrl: "",
          source: "Contributor",
          sourceUrl: "",
